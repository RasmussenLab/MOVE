...
from move.utils.data_utils import generate_file, merge_configs, read_ids
...
def main(base_config: MOVEConfig):
    ...
    logger = get_logger(
        logging_path="./logs/", file_name="01_encode_data.log", script_name=__name__
    )

    # Overriding base_config with the user defined configs.
    cfg = merge_configs(base_config=base_config, config_types=["data"])
