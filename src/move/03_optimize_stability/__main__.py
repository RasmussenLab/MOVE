...
from move.utils.analysis import calculate_latent, get_top10_stability
from move.utils.data_utils import (
    get_data,
    get_list_value,
    make_and_save_best_stability_params,
    merge_configs,
)
...
def main(base_config: MOVEConfig):
    ...
    cfg = merge_configs(
        base_config=base_config, config_types=["data", "model", "tuning_stability"]
    )
    ...
    if len(batch_sizes) == 1:
        ...
        raise (
            "Currently the code is implemented to take take only one value for batch_size"
        )
    ...
    embeddings, latents, con_recons, cat_recons, recon_acc = optimize_stability(
        nHiddens,
        nLatents,
        nDropout,
        nBeta,
    ...
    stability_top10, stability_top10_df = get_top10_stability(
        nHiddens, nLatents, nDropout, nLayers, repeat, latents, batch_sizes, nBeta
    )

    stability_total, rand_index, stability_total_df = calculate_latent(
        nHiddens, nLatents, nDropout, repeat, nLayers, nBeta, latents, batch_sizes
    )
