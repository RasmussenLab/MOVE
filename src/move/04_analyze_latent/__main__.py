...
from move.utils.analysis import (
    calc_categorical_reconstruction_acc,
    calc_continuous_reconstruction_acc,
    get_embedding,
    get_feat_importance_on_weights,
    get_feature_importance_categorical,
    get_feature_importance_continuous,
    get_latents,
    get_pearsonr,
    save_feat_results,
)
...
from move.utils.visualization_utils import (
    embedding_plot_discrete,
    embedding_plot_float,
    plot_categorical_importance,
    plot_continuous_importance,
    plot_reconstruction_distribs,
    visualize_embedding,
    visualize_training,
)
...
def main(base_config: MOVEConfig):
    ...
    logger = get_logger(
        logging_path="./logs/", file_name="04_analyze_latent.log", script_name=__name__
    )

    # Overriding base_config with the user defined configs.
    cfg: MOVEConfig = merge_configs(
        base_config=base_config, config_types=["data", "model", "training_latent"]
    )
    ...
    # Getting the data
    cat_list, con_list, cat_names, con_names, headers_all, drug, drug_h = get_data(
        headers_path,
        interim_data_path,
        categorical_names,
        continuous_names,
    ...
    for feature in features_to_visualize:
        if feature not in np.concatenate(headers_all):
            raise ValueError(
                f"{feature} is not in the headers_all list. It could have been it was not among the features of the input dataset or was filtered out during data processing"
            )
    ...
    latent, latent_var, cat_recon, cat_class, con_recon, loss, likelihood = get_latents(
        best_model, train_loader, 1
    )
    cat_total_recon = calc_categorical_reconstruction_acc(
        cat_shapes, cat_class, cat_recon
    )
    ...
    plot_reconstruction_distribs(
        processed_data_path, cat_total_recon, all_values, all_names
    )
    ...
    for feature in features_to_visualize:
        spear_corr = get_pearsonr(
            feature, embedding, cat_list, con_list, cat_names, con_names
        )
        logger.info(
            f"Pearson correlation for the 1st embedding dim of {feature}: {round(spear_corr[0][0], 3)}, p-value={round(spear_corr[0][1], 3)}"
        )
        logger.info(
            f"Pearson correlation for the 2nd embedding dim of {feature}: {round(spear_corr[1][0], 3)}, p-value={round(spear_corr[1][1], 3)}"
        )
    ...
    (
        ...
        total_diffs_cat_np,
    ) = get_feature_importance_categorical(best_model, train_loader, latent)
    (
        all_diffs_con_np,
        sum_diffs_con_np,
        sum_diffs_con_abs_np,
        total_diffs_con_np,
    ) = get_feature_importance_continuous(best_model, train_loader, mask, latent)
    ...
    get_feat_importance_on_weights(
        processed_data_path, best_model, train_loader, cat_names, con_names
    )
