...
from move.utils.analysis import (
    cal_reconstruction_change,
    get_change_in_reconstruction,
    get_drug_similar_each_omics,
    get_inter_drug_variation,
    identify_high_supported_hits,
    make_files,
    overlapping_hits,
    report_values,
    write_omics_results,
...
from move.utils.visualization_utils import (
    visualize_drug_similarity_across_all,
    visualize_indi_var,
)
...
def main(base_config: MOVEConfig):
    ...
    cfg = merge_configs(
        base_config=base_config, config_types=["data", "model", "training_association"]
    )
    ...
    results, recon_results, groups, mean_bas = read_saved_files(
        nLatents, repeats, processed_data_path, version, drug
    )
    ...
    sig_hits, median_p_val = overlapping_hits(
        nLatents, cor_results, repeats, con_names, drug
    )

    # Getting high supported hits
    # Significant hits across latent spaces
    all_hits, collected_overlap = identify_high_supported_hits(
        sig_hits, drug_h, version, processed_data_path
    )
    ...
    recon_average_corr_new_all = np.load(
        processed_data_path
        + "results/results_confidence_recon_all_"
        + version
        + ".npy",
        allow_pickle=True,
    )
    recon_average_corr_all_indi_new = np.load(
        processed_data_path
        + "results/results_confidence_recon_all_indi_"
        + version
        + ".npy",
        allow_pickle=True,
    ).item()
    ...
    df_indi_var = get_inter_drug_variation(
        con_names,
        drug_h,
        recon_average_corr_all_indi_new,
        groups,
    ...
    visualize_drug_similarity_across_all(
        recon_average_corr_new_all, drug_h, version, processed_data_path
    )
