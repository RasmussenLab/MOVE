...
def identify_associations(config: MOVEConfig):
    ...
    def _bayes_approach(task_config: IdentifyAssociationsBayesConfig):
        # Train models
        ...
        if idx > 0:
            ...
            results = results.merge(a_df, on="feature_a_id").merge(
                b_df, on="feature_b_id"
            )
