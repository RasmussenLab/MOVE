...
from move.utils.model_utils import (
    cal_cat_recon,
    cal_con_recon,
    cal_recon,
    cal_sig_hits,
...
def optimize_reconstruction(
    ...
    (
        ...
        loss_tests,
        likelihood_tests,
    ) = initiate_default_dicts(0, 13)

    # Getting the data
    mask_test, test_loader = dataloaders.make_dataloader(
        cat_list=cat_list_test, con_list=con_list_test, batchsize=1
    )
    test_loader = DataLoader(
        dataset=test_loader.dataset, batch_size=1, drop_last=False, shuffle=False
    )

    results_df = []

    logger.info("Beginning the hyperparameter tuning for reconstruction.\n")
    iters = itertools.product(
        nHiddens, nLatents, nLayers, nDropout, nBeta, batch_sizes, range(repeat)
    )
    for nHidden, nLatent, nl, drop, b, batch_size, r in iters:
        ...
        (
            ...
            loss_test,
            likelihood_test,
        ) = get_latent(best_model, train_loader, test_loader, kld_w)
    ...
    np.save(
        processed_data_path + "hyperparameters/con_recon_benchmark_final.npy",
        con_recons,
    )
    np.save(
        processed_data_path + "hyperparameters/cat_recon_benchmark_final.npy",
        cat_recons,
    )
    np.save(
        processed_data_path + "hyperparameters/recon_acc_benchmark_final.npy", recon_acc
    )
    np.save(
        processed_data_path + "hyperparameters/loss_benchmark_final.npy", loss_train
    )
    np.save(
        processed_data_path + "hyperparameters/likelihood_benchmark_final.npy",
        likelihoods,
    )
    np.save(
        processed_data_path + "hyperparameters/best_epochs_benchmark_final.npy",
        best_epochs,
    )

    np.save(
        processed_data_path + "hyperparameters/test_latent_benchmark_final.npy",
        latents_tests,
    )
    np.save(
        processed_data_path + "hyperparameters/test_con_recon_benchmark_final.npy",
        con_recons_tests,
    )
    np.save(
        processed_data_path + "hyperparameters/test_cat_recon_benchmark_final.npy",
        cat_recons_tests,
    )
    np.save(
        processed_data_path + "hyperparameters/test_recon_acc_benchmark_final.npy",
        recon_acc_tests,
    )
    np.save(
        processed_data_path + "hyperparameters/test_loss_benchmark_final.npy",
        loss_tests,
    )
    np.save(
        processed_data_path + "hyperparameters/test_likelihood_benchmark_final.npy",
        likelihood_tests,
    )

    # print tsv
    results_df.to_csv(
        processed_data_path + "hyperparameters/hyperparameters.results.tsv", sep="\t"
    )
...
def optimize_stability(
    ...
    (
        ...
        los,
        likelihood,
    ) = initiate_default_dicts(1, 7)

    logger.info("Beginning the hyperparameter tuning for stability.")
    iters = itertools.product(
        nHiddens, nLatents, nLayers, nDropout, nBeta, range(repeat)
    )
    for nHidden, nLatent, nl, drop, b, r in iters:
        ...
        (
            ...
            test_loss,
            test_likelihood,
        ) = best_model.latent(test_loader, kld_w)
...
def train_model_association(
    ...
    for nLatent, repeat in iters:
        ...
        (
            ...
            loss,
            likelihood,
        ) = best_model.latent(train_test_loader, kld_w)

        con_recon = np.array(con_recon)
        con_recon = torch.from_numpy(con_recon)

        mean_baseline = get_baseline(
            best_model, train_test_loader, con_recon, repeat=10, kld_w=kld_w
        )
        recon_diff, groups = change_drug(
            best_model, train_test_loader, con_recon, drug, start, end, kld_w
        )
        stat = cal_sig_hits(
            recon_diff, groups, drug, mean_baseline, train_loader.dataset.con_all
        )

        recon_diff_corr = dict()
        for r_diff in recon_diff.keys():
            recon_diff_corr[r_diff] = recon_diff[r_diff] - np.abs(
                mean_baseline[groups[r_diff]]
            )

        # Saving the files
        np.save(output_path / f"results_{nLatent}_{repeat}_{version}", stat)
        np.save(
            output_path / f"recon_results_{nLatent}_{repeat}_{version}",
            np.array(list(recon_diff_corr.values())),
        )
        np.save(output_path / f"mean_bas_{nLatent}_{repeat}_{version}", mean_baseline)
        np.save(
            output_path / f"recon_results_1_{nLatent}_{repeat}_{version}",
            np.array(list(recon_diff.values())),
        )

    np.save(
        output_path / f"results_groups_{version}.npy", np.array(list(groups.values()))
    )
...
def train_model(
    ...
    # Initiate loader
    mask, train_loader = dataloaders.make_dataloader(
        cat_list=cat_list, con_list=con_list, batchsize=batch_size
    )
