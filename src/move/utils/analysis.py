...
def get_top10_stability(
    nHiddens, nLatents, drop_outs, nLayers, repeat, latents, batch_sizes, nBeta
):
    """
        ...
        for r in range(repeat):
            ...
            for index, row in corr.iterrows():
                if r == 0:
                    ...
                    top10_changes[name][index].append(
                        np.mean(abs(old_sum - row[old_pos]))
                    )
...
def calculate_latent(
    nHiddens, nLatents, drop_outs, repeat, nLayers, nBeta, latents, batch_sizes
):
    """
        ...
        for r in range(repeat):
            ...
            for index, row in corr.iterrows():
                if r == 0:
                    ...
                    total_changes[name][index].append(
                        np.mean(abs(old_row - row[old_pos]))
                    )
...
def get_latents(best_model, train_loader, kld_w=1):
    ...
    (
        ...
        loss,
        likelihood,
    ) = best_model.latent(train_test_loader, 1)
...
def calc_continuous_reconstruction_acc(con_shapes, con_recon, train_loader):
    ...
    for s in con_shapes:
        ...
        for n in range(0, con_recon.shape[0]):
            con_no_missing = train_loader.dataset.con_all[
                n, total_shape : (s + total_shape - 1)
            ][train_loader.dataset.con_all[n, total_shape : (s + total_shape - 1)] != 0]
            if len(con_no_missing) <= 1:
                all_val.append(np.nan)
                continue
            con_out_no_missing = con_recon[n, total_shape : (s + total_shape - 1)][
                train_loader.dataset.con_all[n, total_shape : (s + total_shape - 1)]
                != 0
            ]
...
def get_feature_data(
    data_type, feature_of_interest, cat_list, con_list, cat_names, con_names
):
    """
...
def get_pearsonr(
    feature_of_interest, embedding, cat_list, con_list, cat_names, con_names
):
    """
...
def get_feature_importance_categorical(model, train_loader, latent, kld_w=1):
    ...
    for cat_shape in cat_shapes:
        ...
        cat_dataset = np.array(
            cat_dataset.view(cat_in.shape[0], cat_shape[1], cat_shape[2])
        )
        for feature_index in tqdm(range(cat_shape[1])):
            ...
            dataset = dataloaders.MOVEDataset(
                input_cat,
                train_loader.dataset.con_all,
...
def get_feature_importance_continuous(model, train_loader, mask, latent, kld_w=1):
    ...
    return (
        all_diffs_con_np,
        sum_diffs_con_np,
        sum_diffs_con_abs_np,
        total_diffs_con_np,
    )


def save_feat_results(
    path,
    all_diffs,
    sum_diffs,
    sum_diffs_abs,
...
def get_feat_importance_on_weights(path, model, train_loader, cat_names, con_names):
    ...
    # Save the
    tmp_pd = pd.DataFrame(
        np.array(w_sum_cat_final)[feature_order_cat], index=features_w_cat
    )
...
def identify_high_supported_hits(sig_hits, drug_h, version, path):
    ...
    # Save result
    np.save(
        path + "05_identify_associations/results_confidence_" + version + ".npy", result
    )
...
def report_values(
    path, sig_hits, median_p_val, drug_h, all_hits, collected_overlap, con_names
):
    """
    ...
    for i, al_con in enumerate(con_names):
        ...
        df_tmp.T.to_csv(
            path
            + "05_identify_associations/sig_ci_files/"
            + con_names[i]
            + "_p_vals.txt",
            sep="\t",
        )
...
def get_change_in_reconstruction(
    recon_average,
    groups,
    drug,
    drug_h,
    ...
    for l in recon_average.keys():
        for d in recon_average[l].keys():
            ...
            gr = groups[d]
            g = [
                not (np.all(a_s == types[0]) or (np.all(a_s == [0, 0])))
                for a_s in drug[gr, d, :]
            ]
                ...
                if con_names[f] in collected_overlap[drug_h[d]]:
                    if con_names[f] in sig_hits[d][l]:
                        ...
                        for indi in range(tmp_recon.shape[0]):
                            if tmp_vals[indi] != 0:
                                recon_average_corr_all_indi[d][indi][f] += tmp_recon[
                                    indi, f
                                ]
    ...
    for d in recon_average_corr_all.keys():
        ...
        for f in range(len(recon_average_corr_all_indi[d])):
            tmp_recon_average_corr_all_indi_new.append(
                np.array(recon_average_corr_all_indi[d][f])
                / np.array(counts_indi[d][f])
            )

        recon_average_corr_all_indi_new[d] = np.transpose(
            np.array(tmp_recon_average_corr_all_indi_new)
        )
    ...
    np.save(
        path
        + "05_identify_associations/results_confidence_recon_all_"
        + version
        + ".npy",
        recon_average_corr_new_all,
    )
    np.save(
        path
        + "05_identify_associations/results_confidence_recon_all_indi_"
        + version
        + ".npy",
        recon_average_corr_all_indi_new,
    )

    return (recon_average_corr_new_all, recon_average_corr_all_indi_new)


def write_omics_results(
    path,
    up_down_list,
    collected_overlap,
    recon_average_corr_new_all,
    ...
    for i in range(len(con_types)):
        for d in collected_overlap:
            ...
            with open(
                path
                + f"05_identify_associations/{con_types[i]}_"
                + d.replace(" ", "_")
                + ".txt",
                "w",
            ) as o:
            ...
            if con_types[i] in up_down_list:
                ...
                vals = recon_average_corr_new_all[
                    list(drug_h).index(d), np.where(np.isin(con_names, n))[0]
                ]
                up = n[vals > 0]
                down = n[vals < 0]
                with open(
                    path
                    + f"05_identify_associations/{con_types[i]}_up_"
                    + d.replace(" ", "_")
                    + ".txt",
                    "w",
                ) as o:
                ...
                with open(
                    path
                    + f"05_identify_associations/{con_types[i]}_down_"
                    + d.replace(" ", "_")
                    + ".txt",
                    "w",
                ) as o:
...
def make_files(
    ...
    for i, n in enumerate(con_dataset_names):
        ...
        for d in drug_h:
            ...
            sig_data = recon_data_d[
                :, np.where(np.isin(all_db_names, sig_drug_names))[0]
            ]
            g = [
                not (np.all(a_s == types[0]) or (np.all(a_s == [0, 0])))
                for a_s in drug[gr, f, :]
            ]
            con_tmp = con_all[gr]
            con_tmp = con_tmp[g]
            sig_names_sort = np.array(all_db_names)[
                np.where(np.isin(all_db_names, sig_drug_names))
            ]
            ...
            for j in sig_part_df.index:
                ...
                ci = stats.t.interval(
                    0.95,
                    len(data_vals) - 1,
                    loc=np.nanmean(data_vals),
                    scale=stats.sem(data_vals),
                )
                ci_all.append("%.4f [%.4f, %.4f]" % (np.mean(data_vals), ci[0], ci[1]))

            ci_collected.append(ci_all)

        ci_collected_df = pd.DataFrame(
            ci_collected, index=drug_h, columns=sig_names_sort
        )
        ci_collected_df.T.to_csv(
            path
            + "05_identify_associations/"
            + con_dataset_names[i]
            + "_ci_sig_"
            + version
            + ".txt",
...
def get_drug_similar_each_omics(
    con_names,
    con_dataset_names,
    all_hits,
    recon_average_corr_new_all,
