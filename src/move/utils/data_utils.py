...
def merge_configs(base_config, config_types):
    ...
    override_types_str = ", ".join(
        str(override_type) for override_type in override_types
    )
...
def get_data(
    headers_path,
    interim_data_path,
    categorical_data_names,
    continuous_data_names,
    data_of_interest,
):
    """
    ...
    if data_of_interest in categorical_data_names:
        ...
        raise ValueError(
            """In data.yaml file data_of_interest is chosen neither
...
def generate_file(
    var_type, raw_data_path, interim_data_path, headers_path, data_type, ids, na="NA"
):
    """
...
def get_sort_list(results_df):
    ...
    results_df["recon_acc_test_mean"] = results_df["recon_acc_test"].map(
        lambda x: x.mean()
    )
...
def make_and_save_best_reconstruct_params(
    results_df, hyperparams_names, max_param_combos_to_save
):
    ...
    logger.info(
        "Starting calculating the best hyperparameter values for further optimization"
    )

    results_df_sorted = get_sort_list(results_df)
    best_hyperpars_vals_dict = get_best_params(
        results_df_sorted, max_param_combos_to_save, hyperparams_names
    )
    ...
    logger.info(
        f"Saving the best hyperparameter values in tuning_stability.yaml for further optimization:\n \n{OmegaConf.to_yaml(dict(best_hyperpars_vals_dict))}\n"
    )
    logger.warning(
        "Please manually review if the hyperparameter values were selected correctly and adjust them in the tuning_stability.yaml file."
    )
...
def get_best_stability_paramset(stability_df, hyperparams_names):
    ...
    params_to_save = dict()
    stability_df_sorted = stability_df.sort_values("difference", ascending=False).iloc[
        :1
    ]
...
def make_and_save_best_stability_params(results_df, hyperparams_names, nepochs):
    ...
    params_to_save, results_df_sorted = get_best_stability_paramset(
        results_df, hyperparams_names
    )
    ...
    logger.info(
        f"Saving best hyperparameter values in training_latent.yaml: \n\n{OmegaConf.to_yaml(dict(params_to_save))}"
    )
    ...
    # Printing the configuration saved
    logger.info(
        f"Saving best hyperparameter values in training_association.yaml: \n \n{OmegaConf.to_yaml(dict(params_to_save))}"
    )
    logger.warning(
        "Please manually review if the hyperparameter values were selected correctly and adjust them in the training_association.yaml and training_latent.yaml files."
    )


def read_saved_files(nLatents, repeats, path, version, drug):
    results, recon_results, groups, mean_bas = initiate_default_dicts(
        n_empty_dicts=0, n_list_dicts=4
    )
    ...
    for nLatent, repeat in iters:
        result = np.load(
            path
            + f"05_identify_associations/results_{str(nLatent)}_{str(repeat)}_{version}.npy",
            mmap_mode="r",
            allow_pickle=True,
        )
        mean_ba = np.load(
            path
            + f"05_identify_associations/mean_bas_{str(nLatent)}_{str(repeat)}_{version}.npy",
            mmap_mode="r",
            allow_pickle=True,
        )
        recon_result = np.load(
            path
            + f"05_identify_associations/recon_results_{str(nLatent)}_{str(repeat)}_{version}.npy",
            mmap_mode="r",
            allow_pickle=True,
        )
    ...
    groups = np.load(
        path + "05_identify_associations/results_groups_" + version + ".npy",
        mmap_mode="r",
        allow_pickle=True,
    )
