...
def get_latent(best_model, train_loader, test_loader, kld_w):
    ...
    train_test_loader = DataLoader(
        dataset=train_loader.dataset, batch_size=1, drop_last=False, shuffle=False
    )
...
def get_baseline(model, train_loader, con_recon, repeat=25, kld_w=0):
    ...
    for r in range(repeat):
        (
            ...
            test_loss_new,
            likelihood_new,
        ) = model.latent(train_loader, kld_w)
...
def change_drug_atc(
    train_loader, trans_table, con_recon, drug, types=[[1, 0]], data_start=1557
):
    ...
    for feature_index in range(data_shapes[1]):
        ...
        same_atc = [
            ...
            if (v == atc and k != drug_h[feature_index])
        ]
        same = 0
        drug_indexes = []
        if len(same_atc) > 0:
        ...
        for t in types:
            ...
            tmp_data = tmp_data.reshape(
                tmp_data.shape[0], data_shapes[1], data_shapes[2]
            )
            ...
            if same != 0:
                for i in drug_indexes:
                    same_group_tmp = [
                        (np.all(a_s == types[0])) for a_s in drug[:, i, :]
                    ]
                    none_group_tmp = [
            ...
            dataset = dataloaders.MOVEDataset(
                new_data,
                train_loader.dataset.con_all[none_group, :],
...
def change_drug(
    model, train_loader, con_recon, drug, data_start, data_end, kld_w, types=[[1, 0]]
):
    """
...
def cal_sig_hits(
    recon_diff_con_none, none_groups, drug, baseline_mean, con_all, types=[[1, 0]]
):
    """
    Calculates mean means of reconstruction differences when changing a label of a drug
    ...
    for f in range(0, data_shapes[1]):
        if f in none_groups:
            ...
            g = [
                not (np.all(a_s == types[0]) or (np.all(a_s == [0, 0])))
                for a_s in drug[none_groups[f], f, :]
            ]
