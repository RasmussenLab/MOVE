...
def visualize_likelihood(
    path, nLayers, nHiddens, nDropout, nBeta, nLatents, likelihood_tests
):
    ...
    fig, axes = plt.subplots(
        n_rows, n_cols, figsize=(18, 15), sharex=True, sharey=True, frameon=False
    )
            ...
            for drop in nDropout:
                ...
                for b in nBeta:
                    ...
                    a = axes[x, y].plot(
                        nLatents,
                        d,
                        label="Dropout: " + str(drop) + ", beta: " + str(b),
                        linestyle=s,
                        color=c,
                        linewidth=2,
                    )
    ...
    plt.figtext(
        0.78, 0.40, "Number of hidden neurons in each layer", rotation=-90, fontsize=14
    )
    ...
    ax3.title.set_position([0.5, 1.03])
...
def visualize_recon_acc(
    path, nLayers, nHiddens, nDropout, nBeta, nLatents, recon_acc, data_type
):
    ...
    fig, axes = plt.subplots(
        n_rows, n_cols, figsize=(40, 30), sharex=True, sharey=True, frameon=False
    )
    ax3 = fig.add_subplot(111, frameon=False)
    ...
    nBeta = [
        nBeta[0]
    ]  # Beta didn't really effect the results so for the reconstructions we only looked at one value
    batch_size = 10
    # Plotting
    for nl in nLayers:
        x = 0
    ...
    # Adding annotation of variable on right side of the graph
    plt.figtext(
        0.92, 0.45, "Number of hidden neurons in each layer", rotation=-90, fontsize=24
    )
...
def draw_boxplot(path, df, title_text, y_label_text, save_fig_name):
    ...
    ax.set_xticklabels(
        ax.get_xticklabels(), rotation=45, size=16, horizontalalignment="right"
    )
...
def visualize_embedding(
    path, feature_of_interest, embedding, mask, cat_list, con_list, cat_names, con_names
):
    ...
    feature_data, headers = get_feature_data(
        data_type,
        feature_of_interest,
        cat_list=cat_list,
        con_list=con_list,
...
def visualize_indi_var(df_indi_var, version, path):
    ...
    plt.grid(False)
    plt.savefig(
        path + "results/drug_individual_variations_" + version + ".pdf",
        format="pdf",
        dpi=800,
    )


def visualize_drug_similarity_across_all(
    recon_average_corr_new_all, drug_h, version, path
):
